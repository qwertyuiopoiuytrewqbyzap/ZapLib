...
local LoadConfiguration = function()
...
local CachedTime = {}
local CreateAnimation = function(Waiting, Ins, Time, Data)
...
local CallbackCaller = function(Callback, ...)
	if type(Callback) == "function" then
		local Succes, Error = pcall(Callback, ...)
		if not Succes then
...
local NotifyThereads = {}

function Library:Notify(Settings)
	table.insert(NotifyThereads, task.spawn(pcall, function()
...
function Library:CreateWindow(Settings)
	...
	function Current:CreateTab(TabName, TabLogo)
		...
		function TabFunctions:CreateButton(Table)
			...
			Button.MouseButton1Click:Connect(function()
				task.spawn(CallbackCaller, Callback)
		...
		function TabFunctions:CreateToggle(Table)
			...
			task.spawn(CallbackCaller, Callback, EnabledFunction)
		...
		function TabFunctions:CreateSlider(Table)
			...
			task.spawn(CallbackCaller, Callback, NewValue)
		...
		function TabFunctions:CreateInput(Table)
			...
			task.spawn(CallbackCaller, Callback, Library.Flags[Flag])
			...
			Box.FocusLost:Connect(function()
				task.spawn(CallbackCaller, Callback, Box.Text)
			...
			function Table:Set(NewText)
				task.spawn(CallbackCaller, Callback, NewText)
		...
		function TabFunctions:CreateKeybind(Table)
			...
			if not HoldToInteract then
				...
				UserInputService.InputBegan:Connect(function(input)
					if input.KeyCode == ActualKey and tick() - CoolDown > 0.1 then
						task.spawn(CallbackCaller, Callback, true)
				...
				UserInputService.InputEnded:Connect(function(input)
					if input.KeyCode == ActualKey then
						if FirstTouchKey then
							task.spawn(CallbackCaller, Callback, false)
							CreateAnimation(false, KeyBox, 0.25, {TextTransparency = 0.25})
						else
							FirstTouchKey = true
						end
		...
		function TabFunctions:CreateDropdown(Table)
			...
			if CurrentOption then
				task.spawn(CallbackCaller, Callback, CurrentOption)
			end
			...
			local OptionsMaker = function(TableList)
				local Connections = {}
				...
				for _, v in ipairs(TableList) do
					...
					table.insert(OptionConnections, Option.MouseButton1Click:Connect(function()
						if Enabled == true then
							Enabled = false
							if v ~= NewOption then
								task.spawn(CallbackCaller, Callback, v)
			...
			local UpdateCanvas = function(TableList)
				if #TableList < 3 then
		...
		function TabFunctions:CreateMultiDropdown(Table)	
			...
			if PackedCurrentOption then
				task.spawn(CallbackCaller, Callback, PackedCurrentOption)
			end
			...
			OnOffFunction = function()
				...
				if CurrentValue then
					...
					task.spawn(CallbackCaller, Callback, Options)
				else
					PackedCurrentOption = {}
					task.spawn(CallbackCaller, Callback, {})
				end
			...
			local OptionsMaker = function(TableList)
				for _, v in ipairs(TableList) do
					...
					local MultiDropdownFunction = function(AllModifier)
						if Enabled == true then
							Enabled = false
	...
	MainUi.Destroying:Once(function()
		Library.Notify = EmpyFunc

		for _, TabObjectResult in ipairs(CurrentFunctions) do
			TabObjectResult.Set = EmpyFunc
		end
		
		for _, Thread in ipairs(NotifyThereads) do
			pcall(task.cancel, Thread)
		end
		
		table.clear(NotifyThereads)
